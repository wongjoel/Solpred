...
Model 9 - fully conv + physical measurements
...
class FullyConv(solpred_common.SolpredModule):
    def __init__(self, args):
        ...
        self.linear_block = nn.Sequential(
            nn.Linear((7500 + 2 + (3 * args.input_terms)), 1024),
            nn.LeakyReLU(),
            nn.Dropout(0.4),
            nn.Linear(1024, 1024),
            nn.LeakyReLU(),
    ...
    def forward(self, img, in_data, diffuse_direct_irradiance, most_recent_clear_sky, target_clear_sky):
        ...
        conv1_aug = torch.cat((conv1_flat, in_data, diffuse_direct_irradiance, most_recent_clear_sky, target_clear_sky), dim=1)
