...
class Sunset(solpred_common.SolpredModule):
    ...
    def visualise_activations(self, dataloader):
        ...
        for img, in_data, target, diffuse_direct_irradiance, most_recent_clear_sky, target_clear_sky, json_data in dataloader:
            ...
            result = {"actual": target.item(),
                    ...
                    "persist": json_data[0]["inputs"][0][ghi_label]}
            ...
            for col in range(10):
                for row in range(5):
                    index = row + (5 * col) - 1
                    if index <= -1:
                        ax[row, col].imshow(img_HWC.numpy())
                        ax[row, col].axis("off")
                        ax[row, col].set_title("input")
                    elif index >= 48:
                        ax[row, col].axis("off")
                    else:
